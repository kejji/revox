...
const DEFAULT_INTERVAL_MIN = parseInt(process.env.THEMES_DEFAULT_INTERVAL_MINUTES || "1440", 10);

const todayYMD = () => new Date().toISOString().slice(0,10);
const makeJobId = (app_pk, day) => "job_" + crypto.createHash("sha256").update(`${app_pk}|${day}`).digest("hex").slice(0,16);
...
export const handler = async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({ msg: "themes.scheduler.query.ok", count: q.Count || 0, scanned: q.ScannedCount, lastEvalKey: !!q.LastEvaluatedKey }));
  } catch (e) {
    errors++;
    console.log(JSON.stringify({ msg: "themes.scheduler.query.error", error: String(e?.message || e) }));
  ...
  for (const it of q.Items ?? []) {
    ...
    if (it.enabled === false) { console.log(JSON.stringify({ msg: "themes.skip.disabled", app_pk })); continue; }
    ...
    try {
      ...
      locked++;
      console.log(JSON.stringify({ msg: "themes.lock.ok", app_pk, untilIso: new Date(now + LOCK_MS).toISOString() }));
    } catch {
      lockConflicts++;
    ...
    try {
      const r = await sqs.send(new SendMessageCommand({
        QueueUrl: QUEUE,
        MessageBody: JSON.stringify({ app_pk, day, job_id }) // pas de limit/from/to → défaut worker (90j)
      }));
      enqueued++;
      console.log(JSON.stringify({ msg: "themes.sqs.send.ok", app_pk, messageId: r.MessageId, day, job_id }));
    } catch (e) {
      errors++;
      // release lock
      try { await ddb.send(new UpdateCommand({ TableName: TABLE, Key: { app_pk }, UpdateExpression: "REMOVE in_flight_until" })); } catch {}
      console.log(JSON.stringify({ msg: "themes.sqs.send.error", app_pk, error: String(e?.message || e) }));
      continue;
    }
    ...
    try {
      ...
      errors++;
      console.log(JSON.stringify({ msg: "themes.reschedule.error", app_pk, error: String(e?.message || e) }));
    }
  }

  console.log(JSON.stringify({ msg: "themes.scheduler.end", processed, locked, lockConflicts, enqueued, errors }));
